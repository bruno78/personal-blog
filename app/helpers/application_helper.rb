module ApplicationHelper
  class HTMLwithPygments < Redcarpet::Render::HTML
    ...
    def image(link, title, alt_text)
      if title =~ /=(\d+)x(\d+)/
        %(<img src="#{link}" width="#{$1}px" height="#{$2}px" class="img-responsive center" alt="#{alt_text}>")
      elsif title =~ /=(\d+)x/
        %(<img src="#{link}" width="#{$1}px" class="img-responsive center" alt="#{alt_text}>")
      elsif title =~ /=x(\d+)/
        %(<img src="#{link}" height="#{$1}px" class="img-responsive center" alt="#{alt_text}>")
      else
        %(<img src="#{link}" title="#{title}" class="img-responsive center" alt="#{alt_text}">)
      end
    end
